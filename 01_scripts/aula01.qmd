...
O R por si só possui muitas funcionalidades.
...
Mas eu posso fazer gráficos muito mais bonitos e customizados se eu usar pacotes de visualização de dados como o pacote `ggplot2`.
...
Esta base foi desenvolvida por um instituto chamado [Gapminder](https://www.gapminder.org/). Em tradução livre retira do site:
...
> O que estou fazendo? Estou falando: R, acesse um conjunto de dados chamado `gapminder` que está dentro do pacote `gapmider` e guarde em um objeto que eu vou chamar de `dados`.
...
Você também pode usar `glimpse()` para ver um resumo da estrutura dos dados — quais colunas existem e que tipo de informação cada uma guarda.
...
>
> Note também que existem o tipo da variável ao lado do nome. Variáveis `fct` são qualitativas, variáveis `dbl` são quantitativas contínuas e variáveis `int` são quantitativas contínuas.

**Atenção!** É muito importante inspecionar seus dados. Entender bem o que significam. Uma das fontes básicas - além do próprio conjunto de dados - é o dicionário de dados. Lembra da aula do dia 06/04 que vimos os dicionários de algumas bases?
...
| `left_join()` | **Junta** seu conjunto de dados com outro dataset. |
...
```{r}
# Removendo a coluna pop e guardando em um novo objeto chamado dados_sem_pop. Veja que agora não temos mais a variável pop
...
> **Exercício 3:** selecione apenas as colunas `country`, `year`, `continent` e `gdpPercap`. e armazene em um novo conjunto de dados chamado dados_2.

## 5.2. `filter()` — Filtrando linhas

O `filter()` permite que você fique apenas com as linhas que atendem a uma condição.
